...
public interface IContentRepository : IRepository<ContentItem>
{
    ...
    Task<(IReadOnlyList<ContentItem> Items, int Total)> ListAsync(string? contentTypeKey, ContentStatus? status, Guid? categoryId, string? search, int page, int pageSize, CancellationToken ct = default);
}
